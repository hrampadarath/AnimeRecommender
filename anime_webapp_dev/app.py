...
def similar_by_name():
    ...
    for i, name in enumerate(anime_db.name):
        if query.lower() in name.lower():
            info = {
                "name": anime_db.loc[i]['name'],
                "english_name": anime_db.loc[i]['english_name'],
                "rating": round(anime_db.loc[i]['rating'],2),
                "genre": anime_db.loc[i]['genre'],
                "type": anime_db.loc[i]['type'],
                "MAL": anime_db.loc[i]['url']
            }
